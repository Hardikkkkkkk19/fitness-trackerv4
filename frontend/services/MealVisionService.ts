...
Do NOT include any markdown code blocks, text outside the JSON, or other wrapper. Output only raw clean valid parsing-ready JSON.`;
    ...
    // Save to history automatically (asynchronous fire-and-forget style)
    this.saveToHistory(resultPayload);
    ...
    if (!exists) {
      ...
      try {
        ...
        while (slicedHistory.length > 2) {
          ...
          try {
            localStorage.setItem(STORAGE_KEY, JSON.stringify(slicedHistory));
  ...
  async deleteFromHistory(id: string, userId?: string): Promise<MealScanResult[]> {
    try {
      ...
      const uid = userId || session?.user?.id;
      if (uid) {
        // Try to delete both with the text id or uuid depending on the table
